...
const CREDITS = {
  ...
  words: { used: 82400, total: null }, // null = unlimited
    ...
    <div className="mb-5">
      ...
      {pct === null && (
        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
          <div className="h-full w-full rounded-full" style={{ background: "linear-gradient(90deg,#7c3aed44,#06b6d444)" }} />
        </div>
      )}
    </div>
  );
    ...
    <div style={{ animation: "fadeUp 0.4s ease both" }}>
      ...
      <div className="rounded-2xl border border-violet-500/25 p-6 mb-8" style={{ background: "rgba(124,58,237,0.06)" }}>
        <textarea
          ...
          rows={3}
          className="w-full rounded-xl px-4 py-3 text-white placeholder-slate-600 outline-none focus:ring-2 focus:ring-violet-500 transition text-sm resize-none mb-4"
      ...
      {/* Generated result */}
      {generating && (
        <div className="rounded-2xl border border-white/10 flex items-center justify-center mb-8" style={{ height: 260, background: "rgba(255,255,255,0.02)" }}>
      ...
      {generated && !generating && (
        <div className="rounded-2xl border border-violet-500/30 mb-8 overflow-hidden" style={{ animation: "fadeUp 0.4s ease both" }}>
          <div className="relative" style={{ height: 260, background: "linear-gradient(135deg,#1e0a3e,#0a1030,#3a1060)" }}>
          ...
          <div className="flex gap-3 p-4" style={{ background: "rgba(0,0,0,0.3)" }}>
            ...
            <button className="px-4 py-2 rounded-lg text-xs font-bold text-slate-300 border border-white/10 hover:bg-white/5 transition">🔁 Regenerate</button>
          </div>
        </div>
      )}
    ...
    <div style={{ animation: "fadeUp 0.4s ease both" }}>
      ...
      <div className="rounded-2xl border border-cyan-500/25 p-6 mb-8" style={{ background: "rgba(6,182,212,0.05)" }}>
        <textarea
          ...
          placeholder="Describe your video or paste a script… e.g. 'A 30-second product launch reveal for a smart watch, epic cinematic score'"
          rows={4}
          className="w-full rounded-xl px-4 py-3 text-white placeholder-slate-600 outline-none focus:ring-2 focus:ring-cyan-500 transition text-sm resize-none mb-4"
          style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}
        />
      ...
      {done && !processing && (
        <div className="rounded-2xl border border-cyan-500/30 mb-8 overflow-hidden" style={{ animation: "fadeUp 0.4s ease both" }}>
          <div className="relative flex items-center justify-center" style={{ height: 220, background: "linear-gradient(135deg,#0a2040,#062030,#104060)" }}>
            <div className="text-center">
              <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3" style={{ background: "rgba(6,182,212,0.2)", border: "2px solid rgba(6,182,212,0.4)" }}>
...
function ContentSection() {
  ...
  const SAMPLE_OUTPUT = `# The Future of AI-Powered Creativity\n\nArtificial intelligence is no longer a tool reserved for tech giants. Today, independent creators, small studios, and solo entrepreneurs are harnessing the same power...\n\n## Why AI Changes Everything\n\nSpeed, scale, and consistency — the three pillars that once required entire teams can now flow from a single creator with the right platform.\n\n[Continue reading...]`;

  const generate = () => {
    if (!topic.trim()) return;
    setLoading(true); setOutput("");
    setTimeout(() => { setLoading(false); setOutput(SAMPLE_OUTPUT); }, 2000);
  };
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {TYPES.map((t) => (
              <button key={t} onClick={() => setType(t)}
                ...
                style={type === t ? { background: "linear-gradient(90deg,#f59e0b,#ef4444)", color: "#fff" } : { background: "rgba(255,255,255,0.06)", color: "#94a3b8", border: "1px solid rgba(255,255,255,0.1)" }}
              >{t}</button>
            ))}
          </div>
          <input
...
function ChatSection() {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      setThinking(false);
      setMessages((m) => [...m, { role: "ai", text: `Great question! Here's my take on "${userMsg.slice(0, 40)}…" — I'd suggest exploring the core concepts first, then building out from there. Want me to go deeper on any specific aspect?` }]);
    }, 1800);
  };
  ...
  return (
    <div style={{ animation: "fadeUp 0.4s ease both" }} className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4" style={{ maxHeight: 360 }}>
          ...
          {thinking && (
            <div className="flex justify-start">
              <div className="px-4 py-3 rounded-2xl flex items-center gap-2" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}>
                <Loader size="sm" />
...
function CreditsSection() {
  return (
    <div style={{ animation: "fadeUp 0.4s ease both" }}>
      ...
      <div className="rounded-2xl border border-white/8 overflow-hidden" style={{ background: "rgba(255,255,255,0.02)" }}>
        {[
          { date: "Mar 1, 2026", desc: "Pro Plan", amount: "$19.00", status: "Paid" },
          { date: "Feb 1, 2026", desc: "Pro Plan", amount: "$19.00", status: "Paid" },
          { date: "Jan 1, 2026", desc: "Pro Plan", amount: "$19.00", status: "Paid" },
        ].map((b, i) => (
          <div key={i} className="flex items-center px-5 py-4 border-b border-white/5 last:border-0">
            ...
            <span className="text-xs font-bold px-2 py-1 rounded-full text-emerald-400 bg-emerald-400/10">{b.status}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ animation: "fadeUp 0.4s ease both" }}>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl border border-white/8 p-6 flex flex-col items-center text-center" style={{ background: "rgba(255,255,255,0.02)" }}>
          <div
            className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-black text-white mb-4"
            style={{ background: G2 }}
          >
            {USER.avatar}
          </div>
...
export default function Dashboard({ onLogout }) {
  ...
  // Section switch with brief loader
  const [switching, setSwitching] = useState(false);
  const handleNav = (key) => {
    if (key === active) return;
    setSwitching(true);
    setSidebarOpen(false);
    setTimeout(() => { setActive(key); setSwitching(false); }, 350);
  };
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#080614", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Topbar */}
        <header className="flex items-center justify-between px-5 h-16 border-b border-white/5 flex-shrink-0" style={{ background: "rgba(8,6,20,0.8)", backdropFilter: "blur(16px)" }}>
          <div className="flex items-center gap-3">
            <button
              className="md:hidden p-2 rounded-lg border border-white/10 text-slate-400 hover:text-white transition"
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold" style={{ background: "rgba(124,58,237,0.15)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.25)" }}>
        ...
        {/* Page body */}
        <main className="flex-1 overflow-y-auto p-5 sm:p-8" style={{ maxWidth: "100%" }}>
